combo.strip <- function(k, call, put){ # Straddle Combination of Calls
  ...
  DF$`Total Payoff` <- DF[,2] + 2 * DF[,3] # Total Pay-off column
  
  plot(x = DF[,1], y = DF[,4], las = 1, ylab = "Profit ($)", type = "l",
       xlab = "Stock Price ($)", ylim = c(min(DF[,4]), max(DF[,4])), 
       main = "Profit & Loss from Strip", lwd = 5, col = "red")
  ...
  text(x = k + 7.5, y = 0.25 - put,
       sprintf("2 Long Puts, Strike Price of $%s", k)) # Short Put
  text(x = k - 7.5, y = 0.25 - call,
       sprintf("Long Call, Strike Price of $%s", k)) # First Long Put
  
  DF # Display data frame of pay-off
}
combo.strip(70, 1, 3) # Test
