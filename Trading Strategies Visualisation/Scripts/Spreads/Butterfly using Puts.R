spread.butterfly.put <- function(k1, k2, k3, c1, c2, c3){ # Butterfly of puts
  ...
  if (isTRUE(k1 >= k2) && isTRUE(k2 >= k3) && isTRUE(k1 >= k3))
    
    { message("Incorrect values for strike prices") } # Wrong Strike Prices
  
  else { # When all inputs for Strike Prices are correct:
